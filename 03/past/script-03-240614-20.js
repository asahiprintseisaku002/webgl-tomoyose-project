  ...
  init() {
   ...
   moveMoon() {
    ...
    const animate = () => {
        ...
        let radius = this.startPoint.distanceTo(this.endPoint) / 2; // 始点と終点の中間を半径とする

        this.moon.position.x = midPoint.x + radius * Math.cos(angle);
        this.moon.position.y = midPoint.y + radius * Math.sin(angle);
        this.moon.position.z = midPoint.z;
      
        // 次の位置を計算
        let nextPosition = new THREE.Vector3(
            midPoint.x + radius * Math.cos(angle + speed),
            midPoint.y + radius * Math.sin(angle + speed),
            midPoint.z
        );
