...
class ThreeApp {
  ...
  init() {
   ...
   moveMoon() {
    ...
    // 始点から終点への方向ベクトルを計算
    let startToEnd = new THREE.Vector3().subVectors(this.endPoint, this.startPoint).normalize();
    ...
    const animate = () => {
        ...
        // 月のメッシュを円軌道上に移動
        this.moon.position.x = midPoint.x + distance * Math.cos(angle);
        this.moon.position.y = midPoint.y - distance * Math.sin(angle);
        this.moon.position.z = midPoint.z;
      
        // 月が次の位置を常に向くようにする
        let nextPosX = midPoint.x + distance * Math.cos(angle + speed);
        let nextPosY = midPoint.y + distance * Math.sin(angle + speed);
        this.moon.lookAt(new THREE.Vector3(nextPosX, nextPosY, midPoint.z));
