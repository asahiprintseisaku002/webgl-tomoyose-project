...
animate() {
    ...
    if (fraction < 1) {
      // 中間地点を計算
      const midPoint = new THREE.Vector3().lerpVectors(this.startPoint, this.endPoint, 0.5);
      // 始点と終点のベクトルを計算
      const startToEnd = new THREE.Vector3().subVectors(this.endPoint, this.startPoint);
      // 中間地点から垂直に半径分だけ離れた中心点を計算
      const centerPoint = new THREE.Vector3().addVectors(midPoint, new THREE.Vector3(-startToEnd.y, startToEnd.x, 0).normalize().multiplyScalar(this.distance / 2));
      // 始点から中心点までの角度を計算（ラジアン）
      const angle = Math.PI * 2 * fraction; // fraction が 0 から 1 の間で変化するため、これを 2 * PI に掛ける
  
      // 月の新しい位置を計算
      const newPosition = new THREE.Vector3(
        centerPoint.x + Math.cos(angle) * (this.distance / 2),
        centerPoint.y + Math.sin(angle) * (this.distance / 2),
        this.startPoint.z // Z軸は変化させない
      );
