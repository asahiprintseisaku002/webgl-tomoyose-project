...
class ThreeApp {
  ...
  constructor(wrapper) {
    ...
    window.addEventListener('pointermove', (pointerEvent) => {
        ...
        vector.normalize();
        // Y軸の回転角度を計算（-π から π の範囲）
        const rotationX = scaleX * Math.PI / 2 ;
        const rotationZ = scaleX * Math.PI ;
        // オブジェクトのY軸の回転を設定
        this.moon.rotation.x = rotationX;
        this.moon.rotation.y = rotationZ;
    }, false);
    */
  ...
  animate(start, end, moveDuration) {
    const elapsedTime = Date.now() - this.startTime;
    const fraction = elapsedTime / moveDuration;

    if (fraction < 1) {
      // moon を始点から終点に向かって移動させます。
      this.moon.position.lerpVectors(start, end, fraction);
      requestAnimationFrame(() => this.animate(start, end, moveDuration)); // アロー関数を使用して this のコンテキストを保持
    } else {
      // 移動を完了させます。
      this.moon.position.copy(end);
    }
  }
