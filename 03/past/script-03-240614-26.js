...
class ThreeApp {
  ...
  init() {
   ...
   moveMoon() {
    ...
    let distanceY = distanceX; // y軸の半径をx軸と同じに設定
    let offsetDistance = 0.0001; // Earthから離れる距離
    let angle = Math.PI; // 始点から移動を開始するために0から開始
    let speed = 0.02; // 移動速度
    let moonDistance = 2; // Earthの中心から月までの距離
  
    // 始点が終点の右側にあるかどうかをチェック
    let isStartAtRight = this.startPoint.x > this.endPoint.x;
  
    // 始点と終点のベクトルから正規化された法線ベクトルを計算
    let normal = new THREE.Vector3().subVectors(this.endPoint, this.startPoint).normalize().multiplyScalar(offsetDistance);
  
    const animate = () => {
      this.animationId = requestAnimationFrame(animate);
  
      // 角度を更新
      angle += isStartAtRight ? -speed : speed; // 始点が右側にある場合は角度を減少させ、そうでない場合は増加させる
  
      // 月のメッシュを楕円軌道上に移動
      this.moon.position.x = midPoint.x + distanceX * Math.cos(angle) + normal.x;
      this.moon.position.y = midPoint.y + distanceY * Math.sin(angle) + normal.y; // y軸の距離を調整
      this.moon.position.z = midPoint.z + normal.z; // Earthから離れる距離を加える
  
      // 月の位置を正規化し、Earthの中心から一定の距離を保つ
      ...
      // 終点に到達したかチェック
      if (Math.abs(angle) >= Math.PI * 2) { // 一周したらアニメーションを停止
        cancelAnimationFrame(this.animationId);
        this.animationId = undefined;
