...
window.addEventListener('keydown', (keyEvent) => {
    switch (keyEvent.key) {
      case ' ':
        if (this.isDown) {
          // スペースキーが離されたときにカメラの位置を元に戻す
          this.camera.position.copy(this.originalCameraPosition);
          this.isDown = false;
        } else {
          // スペースキーが押されたときにカメラの位置を更新
          this.camera.position.copy(this.camera.position); // カメラを月の位置に移動
          this.camera.lookAt(this.endPoint); // カメラを終点に向ける
          this.isDown = true;
        }
  ...
  init() {
   ...
   moveMoon() {
    ...
    let originalCameraPosition = this.camera.position.clone(); // カメラの元の位置を保存
    ...
    const animate = () => {
      ...
      this.moon.position.y += height * Math.sin(Math.PI * t); // 頂点の高さを調整
