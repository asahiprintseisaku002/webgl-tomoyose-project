  ...
  init() {
   ...
   moveMoon() {
    ...
    const animate = () => {
      ...
      // 月の次の位置を計算
      let nextPosX = midPoint.x + (earthRadius + distanceX) * Math.cos(angle + speed);
      let nextPosY = midPoint.y + (earthRadius + distanceY) * Math.sin(angle + speed);
      let nextPosZ = midPoint.z;
      ...
      this.moon.lookAt(new THREE.Vector3(nextPosX, nextPosY, nextPosZ));
