...
class ThreeApp {
  ...
  init() {
   ...
   setRaycaster(event) {
    ...
    // OrbitControlsを更新してカメラの位置と向きを更新
    ...
    // Earthの現在の回転状態を取得
    //let earthQuaternion = this.earthMesh.quaternion;
    ...
    this.intersects = this.raycaster.intersectObject(this.earth);
      ...
      if (this.intersects.length > 0) {
        ...
        // Earthの回転を考慮して交差点の座標を変換
        //intersectPoint.applyQuaternion(earthQuaternion.inverse());
        ...
        if (!this.isStartPointSet) {
          ...
          this.isStartPointSet = true;
            // OrbitControlsのターゲットを始点に設定
            this.controls.target.copy(this.startPoint);
