...
class ThreeApp {
  ...
  init() {
   ...
   moveMoon() {
    ...
    let distanceY = -distanceX * 3.5; // y軸の半径をx軸の2倍に設定
    ...
    const animate = () => {
        ...
        let nextPosition = new THREE.Vector3(
          midPoint.x + (earthRadius + distanceX) * Math.cos(angle + speed),
          midPoint.y + (earthRadius + distanceY) * Math.sin(angle + speed),
          midPoint.z
        );
      
        // 月が次の位置を常に向くようにする
        this.moon.lookAt(nextPosition);
