...
moveMoon() {
    ...
    let rotationAxis = new THREE.Vector3().crossVectors(new THREE.Vector3(1, 0, 0), startToEnd).normalize();
    ...
    // 月の位置が更新されるたびに呼び出される関数
    const updateMoonDirection = () => {
        // 現在の位置から目的地へのベクトルを計算
        let direction = new THREE.Vector3().subVectors(this.endPoint, this.moon.position).normalize();

        // 月の向きを更新
        this.moon.quaternion.setFromUnitVectors(new THREE.Vector3(0, 0, 1), direction);
    }
