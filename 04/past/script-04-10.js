...
for (let g = 0; g < 4; ++g) {
  ...
  render() {
    ...
    for (let i = 0; i < this.points.geometry.attributes.position.count; i++) {
        ...
        const endPos = this.endPositions[i];

        // 現在の時間に基づいて初期位置と最終位置の間で補間
        const t = (Math.sin(performance.now() * 0.001) + 1) / 2;
        position.setXYZ(i, startPos.lerp(endPos, t).x, startPos.lerp(endPos, t).y, startPos.lerp(endPos, t).z);
    }

    // パーティクルの位置の変更をレンダラーに通知
