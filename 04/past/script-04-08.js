...
for (let g = 0; g < 4; ++g) {
      ...
      for(let j = 0; j <= COUNT; ++j){
        ...
        if (g === 0) {
          ...
          z = 0.0;
            ...
            this.scaleFactor = 5;    
          for(let k = 0; k < this.starVertices.length; k += 3) {
            x = this.starVertices[k] * this.scaleFactor;
            y = this.starVertices[k + 1] * this.scaleFactor;
            z = this.starVertices[k + 2] * this.scaleFactor;
            vertices.push(x, y, z);
          }
        } else if (g === 2) {
          // 形状3
    ...
    for (let n = 0; n< particleCount; n++) {
        // 初期位置
        this.startPositions[n] = new THREE.Vector3(
        ...
        // 最終位置
        this.endPositions[n] = new THREE.Vector3(
            Math.random() * spread - spread / 2,
            Math.random() * spread - spread / 2,
            Math.random() * spread - spread / 2
        );
  ...
  load() {
    return new Promise((resolve) => {
      // 読み込むファイルのパス @@@
      const gltfPath = './star.glb';
      const loader = new GLTFLoader();
      loader.load(gltfPath, (gltf) => {
        // あとで使えるようにプロパティに代入しておく
        this.starGeometry = gltf.scene.children[0].geometry;
        this.starVertices = this.starGeometry.attributes.position.array;
        this.gltf = gltf;
        resolve();
      });
  ...
  render() {
    ...
    for (let n = 0; n < particleCount; n++) {
        // 初期位置と最終位置の間を補間
        var t = (1 + Math.sin(this.time * 0.5)) / 2; // tは0から1の間で変化
        this.particles.geometries.attributes.position.array[n] = this.startPositions[n].lerp(this.endPositions[n], t);
    }

    // パーティクルの位置データを更新
    this.particles.geometries.attributes.position.needsUpdate = true;
