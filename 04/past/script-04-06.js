    ...
    for (let i = 0; i <= COUNT; ++i) {
      ...
      for(let j = 0; j <= COUNT; ++j){
        ...
        if (g === 0) {
          ...
          // 星型
          const radius = 5.0; // 星の大きさを調整
          const middleRadius = 3.5; // 中間の半径を追加
          const innerRadius = 2.0; // 星の内側の半径
          const spikes = 5; // 星のスパイクの数
          let rot = Math.PI / 2 * 3;
          ...
          for(let k = 0; k < spikes; ++k) {
            ...
            y = radius * Math.sin(rot);
            z = 0.0; // z座標を0に設定
            vertices.push(x, y, z);
            rot += step / 2;
        
            // 中間の頂点を追加
            x = middleRadius * Math.cos(rot);
            y = middleRadius * Math.sin(rot);
            z = 0.0; // z座標を0に設定
            vertices.push(x, y, z);
            rot += step / 2;
