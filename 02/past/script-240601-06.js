...
class ThreeApp {
  ...
  static CAMERA_PARAM = {
    ...
    near: 0.1,
    far: 50.0,
    position: new THREE.Vector3(0.0, 2.0, 17.8),
  ...
  static DIRECTIONAL_LIGHT_PARAM = {
    color: 0xffffff,
    intensity: 1.0,
    position: new THREE.Vector3(1.0, 1.0, 20.0),
  };
  ...
  constructor(wrapper) {
    ...
    this.cylinder = new THREE.Mesh(this.cylinderGeometry, this.material5);
    this.cylinder.position.y = 0
    this.group2.add(this.cylinder);

    this.group.position.x = -8;
    this.group.position.y = 2;
    this.group2.position.x = 8;
  ...
  load() {
    return new Promise((resolve) => {
      const imagePaths = ['./r1.jpg', './r2.jpg', './w1.jpg', './w2.jpg'];
      ...
      Promise.all(imagePaths.map(path => {
        return new Promise((resolveTexture) => {
          loader.load(path, (texture) => {
            ...
            const geometryAspect = this.boxGeometry.parameters.width / this.boxGeometry.parameters.height;
  
            // テクスチャの繰り返しを設定してアスペクト比を保持
            texture.repeat.x = imageAspect / geometryAspect;
  ...
  render() {
    ...
    const lightOffset = new THREE.Vector3(0, 0, 10);
