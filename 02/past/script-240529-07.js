...
class ThreeApp {
  ...
  constructor(wrapper) {
    ...
    for (let i = 0; i < this.BC; i++) {
        ...
        this.board = new THREE.Mesh(this.boxGeometry, this.material2);
        this.board.position.x = this.radius * Math.cos(this.angle);
        this.board.position.z = this.radius * Math.sin(this.angle);
        this.board.lookAt(new THREE.Vector3(0, 0, 0)); //原点を向かせる
        this.board.rotateY(Math.PI); //原点を向かせた後に回転
    ...
    this.cylinderGeometry = new THREE.CylinderGeometry( 5.2, 5.2, 0.1, 32 ); 
    this.cylinder = new THREE.Mesh(this.cylinderGeometry, this.material3);
