...
class ThreeApp {
  ...
  constructor(wrapper) {
    ...
    //this.boxGeometry = new THREE.BoxGeometry( 2.6, 4, 0.5 ); 
    //this.board = new THREE.Mesh( this.boxGeometry, this.material ); 
    ...
    for (let i = 0; i < boardCount; ++i) {
        const board = new THREE.Mesh(this.boxGeometry, this.material);
        //torus.rotation.y += torusRotate;
        board.position.x = (Math.random() * 2.0 - 1.0) * transformScale;
        board.position.y = (Math.random() * 2.0 - 1.0) * transformScale;
        board.position.z = (Math.random() * 2.0 - 1.0) * transformScale;
        // シーンに追加するのではなく、グループに追加する @@@
        this.group.add(board);
        this.boxisArray.push(board);
