 ...
 static SATELLITE_TURN_SCALE = 0.1;
  ...
  init() {
    ...
    */
    ...
    //this.mouseDirection = new THREE.Vector2(0.0, 0.0).normalize();
    ...
    this.satellite2.position.set(0.0, 0.0, 0.0);
    ...
    this.group2Direction = new THREE.Vector3(0.0, 0.0, 0.0).normalize();

    this.moonGroup = new THREE.Group();
    this.moonGroup.add(this.moon);
    this.scene.add(this.moonGroup);
    this.moonGroup.position.set(0.0, 0.5, 0.0);
    ...
    this.isDown = false;
     ...
     this.satellite2Direction = new THREE.Quaternion();
   }
  ...
  render() {
    ...
    const currentQtn = this.satellite2.quaternion.clone();

    // moonGroupの位置を向く新たな向きを計算（クォータニオン）
    const targetDirection = new THREE.Vector3().subVectors(this.moonGroup.position, this.satellite2.position);
    targetDirection.normalize();
    const targetQtn = new THREE.Quaternion().setFromUnitVectors(new THREE.Vector3(0, 0, -1), targetDirection);

    // satellite2の向きをtargetDirectionに向けて少しずつ回転させる
    const newQtn = currentQtn.slerp(targetQtn, 0.01);

    // satellite2の向きを更新
    this.satellite2.quaternion.copy(newQtn);
    ...
    this.moonGroup.rotation.x -= 0.01
