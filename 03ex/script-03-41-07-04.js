...
class ThreeApp {
 ...
 static SATELLITE_TURN_SCALE = 0.1;
  ...
  render() {
    ...
    */
    const targetDirection = new THREE.Vector3().subVectors(this.moon.position, this.satellite2.position).normalize();
    const rotationAxis = new THREE.Vector3().crossVectors(this.satellite2Direction, targetDirection).normalize();
    const cosTheta = this.satellite2Direction.dot(targetDirection);
    const theta = Math.acos(cosTheta);
    const quaternion = new THREE.Quaternion().setFromAxisAngle(rotationAxis, theta);
    this.satellite2.quaternion.premultiply(quaternion);
    this.satellite2Direction = targetDirection;
