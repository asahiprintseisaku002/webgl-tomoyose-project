...
class App {
  ...
  animate(time) {
    ...
    if (this.startTime !== null) {
      ...
      this.progress = Math.min(elapsedTime / this.duration, 1.0);
      this.gl.uniform1f(this.uniformLocation.progress, this.progress);
  
      if (elapsedTime >= this.duration) {
        this.startTime = null;  // アニメーション終了
        this.progress = 0.0;    // progressをリセット
  
        // currentTextureIndexを次のテクスチャに更新しループ
        this.currentTextureIndex = (this.currentTextureIndex + 1) % this.textures.length;
  
        // 次のテクスチャを設定
        this.gl.activeTexture(this.gl.TEXTURE0);
        this.gl.bindTexture(this.gl.TEXTURE_CUBE_MAP, this.textures[this.currentTextureIndex]);
        this.gl.uniform1i(this.uniformLocation.texture1, 0);
  
        this.gl.activeTexture(this.gl.TEXTURE1);
        this.gl.bindTexture(this.gl.TEXTURE_CUBE_MAP, this.textures[(this.currentTextureIndex + 1) % this.textures.length]);
        this.gl.uniform1i(this.uniformLocation.texture2, 1);
      }
    }
  ...
  render() {
    ...
    // テクスチャのバインド（前回設定したテクスチャを使用）
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_CUBE_MAP, this.textures[this.currentTextureIndex]);
    gl.uniform1i(this.uniformLocation.texture1, 0);
  
    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_CUBE_MAP, this.textures[(this.currentTextureIndex + 1) % this.textures.length]);
    gl.uniform1i(this.uniformLocation.texture2, 1);
  
    // レンダリングのフラグの状態を見て、requestAnimationFrame を呼ぶか決める
    if (this.isRendering === true) {
      requestAnimationFrame(this.render.bind(this));  // bind(this)を追加
    }
