...
class App {
  ...
  onClick() {
    // クリック時に次のprogressを設定（0から2の範囲に制限）
    this.targetProgress = (this.progress + 1) % 3;
    
    // `reflection` フラグを切り替える（例えば、progressが0.0の時は反射をオフにするなど）
    const reflectionState = this.targetProgress !== 0;
  
    // シェーダー側に `reflection` の状態を送る
    this.gl.uniform1i(this.uniformLocation.reflection, reflectionState);
  ...
  animateProgress() {
    ...
    this.progress += (this.targetProgress - this.progress) * 0.1; // 緩やかな進行
  ...
  render() {
    ...
    gl.uniform1f(this.uniformLocation.u_time, nowTime);
