...
class App {
  ...
  animateProgress(startTime) {
    ...
    const duration = 1.0; // イージングの持続時間（秒）
  
    if (elapsedTime >= duration) {
      this.progress = this.targetProgress;
    } else {
      const easedProgress = easeOutQuad(elapsedTime / duration);
      this.progress = this.initialProgress + (this.targetProgress - this.initialProgress) * easedProgress;
