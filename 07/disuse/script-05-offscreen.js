...
class App {
  ...
  load() {
    return new Promise(async (resolve, reject) => {
      ...
      if (gl == null) {
        ...
        this.offscreenFramebuffer = WebGLUtility.createFramebuffer(gl, this.canvas.width, this.canvas.height);
        // Promsie を解決
        resolve();
      }
    });
  ...
  setupLocation() {
    ...
    this.uniformLocation = {
      ...
      u_texture1: gl.getUniformLocation(this.program, 'u_texture1'),  // テクスチャユニット
      u_texture2: gl.getUniformLocation(this.program, 'u_texture2'),
  ...
  start() {
    ...
    let currentTexture;
    switch (this.currentTextureIndex) {
        case 0:
            currentTexture = this.texture1;
            break;
        case 1:
            currentTexture = this.texture2;
            break;
        case 2:
            currentTexture = this.texture3;
            break;
    }
    ...
    gl.uniform1f(this.uniformLocation.radius, 0.2); // 初期の半径
    gl.uniform1f(this.uniformLocation.width, 0.3); // 初期の幅
  ...
  render() {
    ...
    // オフスクリーンレンダリング用のフレームバッファをバインド
    gl.bindFramebuffer(gl.FRAMEBUFFER, this.offscreenFramebuffer.framebuffer);
    this.setupRendering();

    // オフスクリーンレンダリングの描画
    this.renderOffscreenScene(vp);

    // フレームバッファのバインドを解除
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);

    // オフスクリーンで生成したテクスチャを利用してメインシーンを描画
    gl.activeTexture(gl.TEXTURE0);
    gl.bindTexture(gl.TEXTURE_2D, this.offscreenFramebuffer.texture);
    gl.uniform1i(this.uniformLocation.u_texture1, 0);

    gl.activeTexture(gl.TEXTURE1);
    gl.bindTexture(gl.TEXTURE_CUBE_MAP, this.texture2);
    gl.uniform1i(this.uniformLocation.u_texture2, 1);
    ...
    // メインシーンの描画
    this.renderMainScene(vp, nowTime);
  }
